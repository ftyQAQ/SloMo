...
parser.add_argument("--outputPath", type=str, default="output.mkv", help='Specify output file name. Default: output.mkv')
# parser.add_argument("--extractionDir", type=str, default="", help='zhongjianjieguo')

args = parser.parse_args(['--ffmpeg=G:/SloMo/Super-SloMo-master/ffmpeg/bin/',
                          '--inputPath=G:/SloMo/Adobe240fps/CT_um/tutorial/data/projections2_re/reproj1_uint32_512ss/',
                          '--sf=3','--fps=30',
                          '--checkpoint=G:/SloMo/Super-SloMo-master/mode/SuperSloMo.ckpt',
                          '--outputPath=G:/SloMo/Adobe240fps/CT_um/tutorial/data/projections2_re/reproj1_uint32_512ss_out1/'])
                          # '--extractionDir=G:/SloMo/Adobe240fps/CT_um/tutorial/extractionDir_projections2/']
...
def main():
    ...
    # Load data
    videoFrames = dataloader_00.Video(root=args.inputPath)
    ...
    with torch.no_grad():
        for _, (frame0, frame1) in enumerate(tqdm(videoFramesloader), 0):
            ...
            for batchIndex in range(args.batch_size):
                aa = frame0[batchIndex].detach().numpy()*255.
                # (frame0[batchIndex].detach()).numpy.\
                    # save(os.path.join(args.outputPath, str(frameCounter + args.sf * batchIndex) + ".png"))
                dxchange.write_tiff(aa[0], os.path.join(args.outputPath + 'project_{:04d}.tiff'.format(frameCounter + args.sf * batchIndex)))
            frameCounter += 1
            ...
            for intermediateIndex in range(1, args.sf):
                ...
                for batchIndex in range(args.batch_size):
                    bb= Ft_p[batchIndex].cpu().detach().numpy()*255.
                    dxchange.write_tiff(bb[0], os.path.join(args.outputPath + 'project_{:04d}.tiff'.format(frameCounter + args.sf * batchIndex)))
                frameCounter += 1

            # Set counter accounting for batching of frames
            frameCounter += args.sf * (args.batch_size - 1)
    cc = frame1[-1].detach().numpy()*255.
    dxchange.write_tiff(cc[0], os.path.join(
        args.outputPath + 'project_{:04d}.tiff'.format(frameCounter + args.sf * batchIndex)))
