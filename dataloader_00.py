...
class Video(data.Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        self.dim = int(self.origDim[0] / 4) * 4, int(self.origDim[1] / 4) * 4
    ...
    def __getitem__(self, index):
        ...
        for framePath in [self.framesPath[index], self.framesPath[index + 1]]:
            # Open image using pil.
            image1 = _pil_loader(framePath)
            image1 = np.array(image1)/255.
            image0 = np.zeros((3,512,512))
            image0[0] = image1[:,:,0]
            image0[1] = image1[:, :, 1]
            image0[2] = image1[:, :, 2]
            image0 = image0.astype(np.float32)
            image = torch.tensor(image0).type(torch.FloatTensor)#,resizeDim=self.dim
            # c = image.numpy()
            # Apply transformation if specified.
            # if self.transform is not None:
            #     image3 = self.transform(image1)
            #     b = image3.numpy()
            sample.append(image)
        return sample
