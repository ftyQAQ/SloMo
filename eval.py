...
parser = argparse.ArgumentParser('Evaluate Model by converting a low-FPS video to high-fps')
parser.add_argument("--input", type=str, required=True, help='path to dataset folder containing train-test-validation folders')
parser.add_argument("--checkpoint", type=str, help='path of checkpoint for pretrained model')
parser.add_argument("--output", type=str, help='Path to output file to save')
parser.add_argument("--batch", type=int, default=2, help='Number of frames to process in single forward pass')
parser.add_argument("--scale", type=int, default=4, help='Scale Factor of FPS')
parser.add_argument("--fps", type=int, default=30, help='FPS of output video')

args = parser.parse_args(['--input=G:/SloMo/Adobe240fps/test/to_mov/out_10.mp4',
                          '--scale=3','--checkpoint=G:/SloMo/Super-SloMo-master/mode/SuperSloMo.ckpt','--fps=29',
                          '--output=G:/SloMo/Adobe240fps/test/to_mov/out_10.mkv',])
#'--extractionDir=G:/SloMo/Adobe240fps/test/extractionDir_out_10/'
